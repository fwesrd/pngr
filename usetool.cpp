/* 更新日志 */
/* usetool 4.1
* 静态库不再include头文件usetool.h，无需担心头文件路径改变带来静态库的错误，但可能需要手动添加一些声明在主调函数前
* 删除不必要的framework.h、pch.h、pch.cpp
* 空项目在属性-常规改为静态库
*/

/* usetool 5.0
* 全新的版本，非常小而精巧，删除很多不必要的代码，提高了实用性
* 删除对纯C的支持，即无extern C
* 删除链表、串
* 完全删除对Easyx库的支持
* 删除小数和字符串的转换
...
* 删除C文件读取字符串函数
* GetTime函数返回时间结构体
* Ramdom函数返回long long类型
* 新增的文件系统
* to_string 转为字符串
* 分离了一些头文件，更加简洁
* 静态库路径文件里的_DEBUG宏自动选择Debug或Release版本
* 所有头文件include在useinclude.h中
* 类从usetool.h头文件分离，但也include类头文件以与以前一样使用。类声明放在useclass.h中，类实现放在useclass.cpp中，模板的实现放在usetemplate中
* window GUI类
*/
...
* 增加字符串UI类
* 取消<lib_path.h>
* usetool.cpp还是include usetool.h，但是条件编译防止在cpp链接库文件
* 遍历目录使用__int64的文件大小
* 内联函数替代宏函数
* 四种类型转换
* 全局函数、文件接口类、字符串UI类、视窗UI类开分不同的文件
*/

/* usetool6.1
* 恢复对多种视窗GUI控件支持
* 取消接口类的继承嵌套，取而代之是用宏定义去屏蔽接口类不需要看到的代码
* 全英语注释
*/
...
* 取消默认使用std命名空间
* 增加CNumber类
* 新增全局函数以初始化COM库，以解决CComPtr<IWICImagingFactory>.p->Release()报错
*/

/* usetool7.1
* IWinUI增加Direct3D
* 增加CMouse类处理鼠标滑轮和鼠标移动
* 整理优化IWinUI
*/
